...
//
        ...
        ScrollView{
            LazyVGrid(columns: columns, spacing: 10) { // Use LazyVGrid for vertical layout
                ForEach(cards.indices, id: \.self) { index in
                    CardView(card: cards[index], isFaceDown: cards[index].isFaceDown, onTap: {
    ...
    private func checkForMatch() {
        ...
        if cards[firstIndex].emoji != cards[secondIndex].emoji {
            // Flip cards back after a delay if they dont match
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) { withAnimation {
                self.cards[firstIndex].isFaceDown = true
                self.cards[secondIndex].isFaceDown = true
            }
                // Update the whole array to ensure UI updates
                self.cards = self.cards.map{ $0 }
            }
        } else {
            // Mark cards as matchd
            cards[firstIndex].isMatched = true
            cards[secondIndex].isMatched = true
        }
        // Clear selected indices regardless of matche
        selectedIndices.removeAll()
    }
    
    // Function to reset the game
    private func resetGame() {
        // Reset all cards to face down and unmatch
        ...
        // Shuffle the cards after all are faced down
